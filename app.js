...
app.get("/", (req,res) =>{
  Post.find(function(err, posts){
    if (err){
      ...
      res.render("home", {paragraphHome: homeStartingContent, post: posts});
    }
  });
});
...
app.get("/post/:postTopic", (req, res) => {
  ...
  const requestedID = req.params.postTopic;

  Post.findById({_id: requestedID}, function(err, posts){
    if (err){
      console.log(err);
    } else {
      res.render("post", {postTilteReq: posts.title, postContentReq: posts.content});
    }
  });
});
